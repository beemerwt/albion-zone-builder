...
export default function App() {
  ...
  const zoneByName = useMemo(() => Object.fromEntries(zones.map((z) => [String(z.name ?? z.id ?? ''), z])), [zones]);
  const zone = zoneByName[zoneName] as Zone | undefined;

  const zoneOptions = Object.keys(zoneByName).sort().map((v) => ({ value: v, label: v }));
  const zoneIdOptions = zones.map((z) => String(z.id ?? '')).filter(Boolean).sort().map((v) => ({ value: v, label: v }));
  ...
  const syncZone = (nextRows: PortRowState[]) => {
    ...
    for (const r of nextRows) {
      ...
      if (!n) continue;
      ports[n] = { x: clamp01(r.x), y: clamp01(r.y), connectsTo: { zoneId: r.zoneId ?? '', port: (r.port ?? '').toUpperCase() } };
    }
  ...
  return <div className="vh-100 d-flex flex-column">
    <Toolbar
      ...
      onWorld={async (file) => {
        ...
        setWorld(parsed);
        const first = (parsed.zones.map((z) => String(z.name ?? z.id ?? '')).filter(Boolean).sort()[0]) ?? '';
      ...
      onImage={(file) => {
        ...
        img.onload = async () => {
          ...
          try {
            ...
            setStatus(`Detected map bounds (+${detected.positiveLineCount}/-${detected.negativeLineCount}).`);
          } catch {
    ...
    <div className="flex-grow-1 d-flex overflow-hidden">
      <div className="border-end p-2" style={{ width: 620, overflow: 'auto' }}>
        ...
        <SearchableDropdown options={zoneOptions} value={zoneName} onChange={onSelectZone} isDisabled={!world} />
        <div className="mt-2">
          <PortsEditor
            rows={rows}
            selecting={selecting}
            ...
            onAdd={() => {
              const next = [...rows, { key: crypto.randomUUID(), name: 'NE', x: 0, y: 0, zoneId: '', port: '' }];
              setRows(next); syncZone(next);
            }}
            onChange={(index, row) => {
      ...
      <div className="flex-grow-1">
        <MapCanvas
          ...
          onClick={(ix, iy) => {
            ...
            const next = rows.map((r, i) => i === selecting ? { ...r, x: clamp01(u), y: clamp01(v) } : r);
            setRows(next); setSelecting(null); syncZone(next);
          }}
        />
      </div>
    </div>
